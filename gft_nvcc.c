 ...
 *
 */
//to fix overloaded function of log2,pow,round,zxf,20151015  
...
//		win[i] = abs(freq)/sqrt(2*PI)*exp(-pow((x-0.5),2)*pow(freq,2)/2.0);
		win[i] = abs(freq)/sqrt(2*PI)*exp(-pow((x-0.5),2)*pow(freq*1.0,2)/2.0);
		win[i+1] = 0.0;
		sum += win[i];
	}
...
int gft_1dSizeOfPartitions(unsigned int N) {
	//return round(mylog2(N))*2+1;
	return round(mylog2(N)*1.0)*2+1;
}
...
int *gft_1dPartitions(unsigned int N) {
	...
	//partitions = (int *)malloc(sizeof(int)*round(mylog2(N))*2+1);
	partitions = (int *)malloc(sizeof(int)*round(mylog2(N)*1.0)*2+1);
	//pOff = round(mylog2(N))*2-1;
	pOff = round(mylog2(N)*1.0)*2-1;
